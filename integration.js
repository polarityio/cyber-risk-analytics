...
function doLookup(entities, options, cb) {
    ...
    getAuthToken(options, (err, token) => {
        ...
        async.each(entities, (entityObj, next) => {
      ...
      cb(err, results);
      Logger.trace({results:results}, "Checking the final results before the callback");
    });
    });
}
...
function _lookupEntity(entityObj, token, options, cb) {
  ...
  Logger.trace({requestoptions: requestOptions}, "Checking the request options coming through");
  ...
  requestWithDefaults(requestOptions, function(err, response, body) {
    ...
    if(typeof(body.total_entries) !== 'undefined') {
      if(body.total_entries === 0 || _.isEmpty(body.incidents)){
        cb(null, {
          entity: entityObj,
          data: null // this entity will be cached as a miss
        });
    ...
    if(entityObj.type === "domain"){
      var domainData = body;
      var severityData = []
      domainData.incidents.forEach(function(item) {
        if(item.severity_score !== undefined && item.severity_score !== null){
          severityData.push(item.severity_score);
      }
    });
      var severity = ["Highest Severity Breach Score: " + Math.max(...severityData)];
    }

    if(entityObj.type === "email"){
      var emailData = body;
    }
